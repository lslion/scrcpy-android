...
import com.dao.ioe.scrcpy.wrappers.ServiceManager;
...
public final class Device {
    ...
    private ScreenInfo computeScreenInfo(int maxSize) {
        logd("screeninfo", "computeScreenInfo is: " + maxSize);
        ...
        logd("screeninfo", "device size width is: " + deviceSize.getWidth());
        logd("screeninfo", "device size width is: " + deviceSize.getWidth());

        int w = deviceSize.getWidth() & ~7; // in case it's not a multiple of 8
        int h = deviceSize.getHeight() & ~7;
        logd("screeninfo", "device size width w is: " + w);
        logd("screeninfo", "device size width h is: " + h);
        ...
        if (maxSize > 0) {
            ...
            if (major > maxSize) {
                logd("screeninfo", "magic > maxSize:" + major);
        ...
        logd("screeninfo", "videoSize is: " + w + ":" + h) ;
...
//        Point point = position.getPoint();
        int scaledX = point.x * deviceSize.getWidth() / videoSize.getWidth();
        int scaledY = point.y * deviceSize.getHeight() / videoSize.getHeight();
        logd("point", "width x is: " + point.x + ":" + deviceSize.getWidth() + ":" + videoSize.getWidth());
        logd("point", "hight x is: " + point.y + ":" + deviceSize.getHeight() + ":" + videoSize.getHeight());
        return new Point(scaledX, scaledY);
    }
