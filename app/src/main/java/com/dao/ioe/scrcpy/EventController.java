...
import com.dao.ioe.scrcpy.wrappers.InputManager;
        ...
        while (true) {
            ...
            if (buffer != null) {
                ...
                if (buffer[2] == 0 && buffer[3] == 0) {
                    ...
                    int action = buffer[0];
                    logd("point", "receive action is: " +action);
                    if (action == MotionEvent.ACTION_UP && (!device.isScreenOn() || proximity)) {
                        if (hit) {
                            logd("point", "receive hit");
                            if (now - then < 250) {
                                then = 0;
                                hit = false;
                                logd("point", "receive hit 111");
                                injectKeycode(KeyEvent.KEYCODE_POWER);
                            } else {
                                logd("point", "receive hit 222");
                                then = now;
                            }
                        } else {
                            logd("point", "receive hit false");
                            hit = true;
                            then = now;
                        }
                        ...
                        if (action == MotionEvent.ACTION_DOWN) {
                            logd("point", "receive ACTION_DOWN 000");
                            lastMouseDown = now;
                        }

                        logd("point", "receive ACTION_DOWN");
                        int button = buffer[1];
                        int X = buffer[2];
                        int Y = buffer[3];
